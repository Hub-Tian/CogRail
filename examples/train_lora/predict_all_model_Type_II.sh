...
DATASET="${DATASET:-Cog-MRSI_RailThreat_masked_Type_II_test}"
SAVE_DATASET="${SAVE_DATASET:-Cog-MRSI_RailThreat_masked_Type_II_MRSI_sft_$(date +%Y%m%d_%H%M%S)}"
DATA_FILE="${DATA_FILE:-/data/Projects/SongZH/Cog-MRSI/data_after_deal_masked_obstacle/Type_II/test/QAs/Cog-MRSI_RailThreat_Type_II_test.json}"
...
run_predict() {
    ...
    local model_path="$2"
    local template="$3"
    local adapter_path="saves/${model_name}/lora/MRSI_sft_II"
